...
function main($request, $payload, $kwargs=array()) {
	...
	$sec = $s;
	if( array_key_exists( $sec->slug, $validNavItems ) ) {
		$payload['active'] = $validNavItems[$sec->slug];
	} elseif( array_key_exists( $sec->sidebar, $validNavItems ) ) {
		$payload['active'] = $validNavItems[$sec->sidebar];
	} elseif( array_key_exists( $sec->template, $validNavItems ) ) {
		$payload['active'] = $validNavItems[$sec->template];
	}
...
function xml($request, $payload, $kwargs=array()) {
	$tpl = new Template('section/main.xml');
	$res = new Response;
	$res->content = $tpl->render($payload);
